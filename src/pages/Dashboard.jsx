...
import {
  ListChecks, PlusCircle, Bot, AppWindow, ArrowRight, FileText, Receipt, Sparkles
} from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard icon={ListChecks} label="Total tasks" value={loading ? '—' : total} accent="brand" />
        <StatCard icon={Sparkles}  label="Completed"   value={loading ? '—' : completed} accent="emerald" />
        <StatCard icon={Bot}       label="Failed"      value={loading ? '—' : failed} accent="amber" />
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <QuickAction to="/tasks/new" icon={PlusCircle} title="New task" description="Run summary or invoice review" />
          <QuickAction to="/tasks" icon={ListChecks} title="Task history" description="View past executions" />
          <QuickAction to="/agents" icon={Bot} title="Run an agent" description="DocumentAnalyst, InvoiceAuditor" />
          <QuickAction to="/apps" icon={AppWindow} title="AI-Pass apps" description="Browse modules" />
        </div>
      </div>
        ...
        <Card>
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <Link to="/tasks/new" className="mt-3 inline-block text-sm font-medium text-brand-600 hover:text-brand-700">
            ...
            <ul className="divide-y divide-gray-200">
              {recent.map(t => {
                ...
                return (
                  <li key={t.id}>
                    <Link to={`/tasks/${t.id}`} className="flex items-center gap-4 p-4 hover:bg-gray-50">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-gray-500">
                          {t.taskType} · {new Date(t.createdAt).toLocaleString()}
                        </div>
                      </div>
